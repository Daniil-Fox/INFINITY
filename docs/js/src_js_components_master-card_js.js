"use strict";
(self["webpackChunkgulp_builder"] = self["webpackChunkgulp_builder"] || []).push([["src_js_components_master-card_js"],{
...
(function initMasterCard() {
  ...
  const gui = typeof window !== "undefined" ? window.__infinityDebugGui || (window.__infinityDebugGui = new lil_gui__WEBPACK_IMPORTED_MODULE_3__.GUI({
    title: "Infinity Debug"
  })) : null;
  if (gui) {
  ...
  const maxRotation = three__WEBPACK_IMPORTED_MODULE_0__.MathUtils.degToRad(8);
  ...
  const ambientLight = new three__WEBPACK_IMPORTED_MODULE_0__.AmbientLight(cardLightParams.ambientColor, cardLightParams.ambientIntensity);
  scene.add(ambientLight);
  const keyLight = new three__WEBPACK_IMPORTED_MODULE_0__.DirectionalLight(cardLightParams.keyColor, cardLightParams.keyIntensity);
  keyLight.position.set(cardLightParams.keyX, cardLightParams.keyY, cardLightParams.keyZ);
  scene.add(keyLight);
  const rimLight = new three__WEBPACK_IMPORTED_MODULE_0__.DirectionalLight(cardLightParams.rimColor, cardLightParams.rimIntensity);
  rimLight.position.set(cardLightParams.rimX, cardLightParams.rimY, cardLightParams.rimZ);
  scene.add(rimLight);
  const fillLight = new three__WEBPACK_IMPORTED_MODULE_0__.DirectionalLight(cardLightParams.fillColor, cardLightParams.fillIntensity);
  fillLight.position.set(cardLightParams.fillX, cardLightParams.fillY, cardLightParams.fillZ);
  scene.add(fillLight);
  let cardModel;
  const cardParams = {
    positionX: 0,
  ...
  const applyCardParams = () => {
    if (!cardModel) return;
    cardModel.position.set(cardParams.positionX, cardParams.positionY, cardParams.positionZ);
    cardModel.rotation.set(cardParams.rotationX, cardParams.rotationY, cardParams.rotationZ);
  ...
  const setupCardGui = () => {
    ...
    positionFolder.add(cardParams, "positionX", -5, 5, 0.01).name("X").onChange(applyCardParams);
    positionFolder.add(cardParams, "positionY", -5, 5, 0.01).name("Y").onChange(applyCardParams);
    positionFolder.add(cardParams, "positionZ", -5, 5, 0.01).name("Z").onChange(applyCardParams);
    const rotationFolder = cardFolder.addFolder("Поворот");
    rotationFolder.add(cardParams, "rotationX", -Math.PI * 2, Math.PI * 2, 0.01).name("X (рад)").onChange(applyCardParams);
    rotationFolder.add(cardParams, "rotationY", -Math.PI * 2, Math.PI * 2, 0.01).name("Y (рад)").onChange(applyCardParams);
    rotationFolder.add(cardParams, "rotationZ", -Math.PI * 2, Math.PI * 2, 0.01).name("Z (рад)").onChange(applyCardParams);
    cardFolder.add(cardParams, "scale", 0.1, 3, 0.01).name("Масштаб").onChange(applyCardParams);
    cardFolder.open();
    positionFolder.open();
    rotationFolder.open();
    const lightsFolder = gui.addFolder("Освещение (Master)");
    const ambientFolder = lightsFolder.addFolder("Рассеянный свет");
    ambientFolder.add(cardLightParams, "ambientIntensity", 0, 5, 0.05).onChange(value => {
      ambientLight.intensity = value;
    });
    ambientFolder.addColor(cardLightParams, "ambientColor").onChange(value => {
      ambientLight.color.set(value);
    });
    const keyFolder = lightsFolder.addFolder("Ключевой свет");
    keyFolder.add(cardLightParams, "keyIntensity", 0, 5, 0.05).onChange(value => {
      keyLight.intensity = value;
    });
    keyFolder.addColor(cardLightParams, "keyColor").onChange(value => {
      keyLight.color.set(value);
    });
    keyFolder.add(cardLightParams, "keyX", -10, 10, 0.1).name("X").onChange(value => {
      keyLight.position.x = value;
    });
    keyFolder.add(cardLightParams, "keyY", -10, 10, 0.1).name("Y").onChange(value => {
      keyLight.position.y = value;
    });
    keyFolder.add(cardLightParams, "keyZ", -10, 10, 0.1).name("Z").onChange(value => {
      keyLight.position.z = value;
    });
    const rimFolder = lightsFolder.addFolder("Контровой свет");
    rimFolder.add(cardLightParams, "rimIntensity", 0, 5, 0.05).onChange(value => {
      rimLight.intensity = value;
    });
    rimFolder.addColor(cardLightParams, "rimColor").onChange(value => {
      rimLight.color.set(value);
    });
    rimFolder.add(cardLightParams, "rimX", -10, 10, 0.1).name("X").onChange(value => {
      rimLight.position.x = value;
    });
    rimFolder.add(cardLightParams, "rimY", -10, 10, 0.1).name("Y").onChange(value => {
      rimLight.position.y = value;
    });
    rimFolder.add(cardLightParams, "rimZ", -10, 10, 0.1).name("Z").onChange(value => {
      rimLight.position.z = value;
    });
    const fillFolder = lightsFolder.addFolder("Заполняющий свет");
    fillFolder.add(cardLightParams, "fillIntensity", 0, 5, 0.05).onChange(value => {
      fillLight.intensity = value;
    });
    fillFolder.addColor(cardLightParams, "fillColor").onChange(value => {
      fillLight.color.set(value);
    });
    fillFolder.add(cardLightParams, "fillX", -10, 10, 0.1).name("X").onChange(value => {
      fillLight.position.x = value;
    });
    fillFolder.add(cardLightParams, "fillY", -10, 10, 0.1).name("Y").onChange(value => {
      fillLight.position.y = value;
    });
    fillFolder.add(cardLightParams, "fillZ", -10, 10, 0.1).name("Z").onChange(value => {
  ...
  const modelPath = themeUrl ? `${themeUrl}/assets/models/gltf/card/Card.gltf` : "../models/gltf/card/Card.gltf";
  ...
  const tick = () => {
    ...
    cardGroup.position.x += (currentX * maxTranslation - cardGroup.position.x) * 0.1;
    cardGroup.position.y += (-currentY * maxTranslation - cardGroup.position.y) * 0.1;
...
//# sourceMappingURL=src_js_components_master-card_js.js.map